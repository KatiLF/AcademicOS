...
export default function Lobby() {
    ...
    //Esto se verifica por medio del email del creado de la etiqueta
    ...
    */
    ...
    async function createEtiqueta(event) {
        ...
        try {
            ...
            const email = Cookies.get('email'); // Obtener el valor de la cookie que contiene el email

            // Agregar el email al objeto formValues
            //Aqui estoy enviando los datos del formulario y el email del que crea la etiqueta
            //la logica cuando llega al server es const { etiqueta, description , email} = req.body;
                        ...
                        <tbody>
                            {data.map((etiqueta) => (
                                <tr key={etiqueta.id}>
                                    ...
                                    {(userRole === 'Administrador' || (userRole === 'Profesor' && userEmail === etiqueta.email)) && (
